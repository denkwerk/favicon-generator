...
<p align="center">
  ...
  Favicons, an Apple touch icon and ready-to-paste <code>&lt;head&gt;</code> tags, plus a web app manifest, theme color<br>
  and Windows tiles when you want them. Generated by a native Rust binary, from an image file or straight from Figma.
</p>
...
- 🎯 **Sensible by default**: `favicon.ico`, an SVG favicon, a PNG and an Apple touch icon, no placeholder values
- 🧩 **Opt-in extras**: web app manifest, theme color, Windows tiles and legacy sizes, each turned on by configuring it
...
Then paste the generated `public/favicons/favicon.html` into your `<head>`. Everything else is optional: a manifest,
a theme color, Windows tiles and more are described under [Configuration](#configuration).
...
By default, only what the image alone provides:
...
Anything that needs values the image can't provide, like an app name or colors, is only added when you configure it:

| You configure | You also get |
| --- | --- |
...
The optional outputs are **groups**. Each one is off until you configure it: set it to `true` for its defaults, or to
an object with its options. `false` turns it off, and so does the matching `--no-…` flag.
...
Adds `manifest.json`, `favicon-192x192.png`, `favicon-512x512.png` and `<link rel="manifest">`. It is written with only
the fields you set, so nothing is made up. Browsers need a `name` or `shortName` to offer installing the app.
...
| `tileColor` | `windows.tileColor` (`browserconfig.xml` is now opt-in) |
